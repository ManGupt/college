...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        c1 = self.names_to_nodes[a]
        c2 = self.names_to_nodes[b]
        if c1 == c2:
            return (-1,0)
        c1p_list = []
        c1p = c1.get_parent()
        ...
        while c2p != None:
            c2p_list.append(c2p)
            c2p = c2p.get_parent()
        degree = len(c1p_list) - len(c2p_list)
        cousin = 0
        
##        c1p_list = c1p_list.reverse()
##        c2p_list = c2p_list.reverse()
        if c1p_list == [] or c2p_list == [] :
            cousin = degree - 1
            return (cousin,abs(degree))
        for c in c1p_list :
            if c in c2p_list :
                pass
            else :
...
t, r = f.cousin("g", "o")
print "'o' is a", words[t], "cousin", r, "removed from 'g'"
